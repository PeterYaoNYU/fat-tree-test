...
# Node switch4: the switch between siwthc 2 and node 3 and 4. 
node_switch4 = request.RawPC('switch4')
iface9 = node_switch4.addInterface('interface-3')
iface10 = node_switch4.addInterface('interface-8')
...
# link-4: connects swithc 3 and node 2
link_4 =  request.Link('link-4')
link_4.Site('undefined')
link_4.addInterface(iface12)
...
#   - switch 3 (iface2): 10.0.2.2/29
#   - switch 4 (iface10): 10.0.2.3/29
iface7.addAddress(pg.IPv4Address("10.0.2.1", "255.255.255.0"))
iface11.addAddress(pg.IPv4Address("10.0.2.2", "255.255.255.0"))
iface10.addAddress(pg.IPv4Address("10.0.2.3", "255.255.255.0"))

# For link-3 (10.0.3.0/24): connecting switch 1 and switch 2
iface8.addAddress(pg.IPv4Address("10.0.3.1", "255.255.255.0"))
iface6.addAddress(pg.IPv4Address("10.0.3.2", "255.255.255.0"))


# for link connecting switch 3 to node 2: Link-4
iface12.addAddress(pg.IPv4Address("10.0.4.1", "255.255.255.0"))
iface2.addAddress(pg.IPv4Address("10.0.4.2", "255.255.255.0"))
...
sudo sysctl -w net.ipv4.ip_forward=1
"""))
node_switch4.addService(pg.Execute(shell="bash", command="""
sudo sysctl -w net.ipv4.ip_forward=1
"""))
...
sudo ip route add 10.0.0.3 via 10.0.0.1
"""))
...
sudo ip route add 10.0.2.0/24 via 10.0.0.1
sudo ip route add 10.0.3.0/24 via 10.0.0.1
sudo ip route add 10.0.0.2 via 10.0.0.1
"""))

node_2.addService(pg.Execute(shell="bash", command="""
sudo ip route add 10.0.0.0/24 via 10.0.4.1
sudo ip route add 10.0.1.0/24 via 10.0.4.1
sudo ip route add 10.0.2.0/24 via 10.0.4.1
sudo ip route add 10.0.3.0/24 via 10.0.4.1
"""))
...
sudo ip route add 10.0.1.3 via 10.0.1.1
"""))
...
sudo ip route add 10.0.2.0/24 via 10.0.1.1
sudo ip route add 10.0.3.0/24 via 10.0.1.1
sudo ip route add 10.0.1.2 via 10.0.1.1
"""))
...
# this command requires the device name, should probably be run after the init. 
# can come up with a way to get the device name from the ifconfig output to automate this process. 
# sudo ip route add 10.0.2.0/29 via 10.0.2.1 dev *** onlink
